...
_PRECAUTIONS: dict[str, list[str]] = {
    ...
    "medium": [
        ...
        "Purchase travel insurance with medical cover",
        "Register with your embassy",
    ],
    "high": [
        ...
        "Secure travel insurance with evacuation cover",
        "Register your trip with your government's travel portal",
    ],
    "critical": [
        "Only travel if absolutely essential",
        "Seek professional security consultancy",
        "Ensure evacuation insurance is in place",
        "Do not travel alone",
    ],
}
...
class SafetyReasoner(BaseReasoner):
        ...
        **_,
        ...
        if city is None:
            return self._not_found(
                destination, f"'{destination}' is not in the knowledge graph."
            )
        ...
        return ReasoningResult(
            ...
            data={
                ...
                "risk_summary": _RISK_SUMMARY.get(
                    safety_level, "Safety level unknown."
                ),
                "precautions": _PRECAUTIONS.get(safety_level, _PRECAUTIONS["low"]),
                "embassy_hint": _EMBASSY_HINT.format(
                    country=country.name if country else "destination"
                ),
                "visa": {
                    "requirement": visa.requirement
                    if visa
                    else "Unknown — verify with your embassy",
                    "max_stay_days": visa.max_stay_days if visa else None,
                    "notes": visa.notes
                    if visa
                    else "No visa data in Sprint 1 for this passport/destination pair.",
                }
                if True
                else None,
            ...
            limitations=[
                "No live FCDO/State Dept feed; safety levels are static Sprint 1 data"
            ],
        )
    ...
    def _visa(self, from_iso: str, to_iso: str):
        ...
        return next(
            (
                v
                for v in visa_reqs
                if v.from_country_iso == from_iso and v.to_country_iso == to_iso
            ),
            None,
        )

    def _languages(self, country) -> list[dict]:
        langs = self._ks.get_connected_entities(
            country.id, "Language", RelationshipType.SPEAKS, "outbound"
        )
        return [
            {"name": lang.name, "iso": lang.iso_code, "native": lang.native_name}
            for lang in langs
        ]
    ...
    def _health_tips(self, country) -> list[str]:
        ...
        if country.continent == "Africa":
            tips += [
                "Consult a travel health clinic about malaria prophylaxis.",
                "Consider yellow fever vaccination (may be required).",
            ]
        if country.safety_level in ("medium", "high", "critical"):
            tips.append(
                "Confirm your travel insurance covers emergency medical evacuation."
            )
        return tips
