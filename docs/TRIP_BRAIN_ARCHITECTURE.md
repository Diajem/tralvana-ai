...
- Trip Brain does not talk to a provider, mock or otherwise — that stays entirely inside each Discovery module, now optionally routed through the Intelligence Gateway (T-025, `travelos/intelligence_gateway/`, `docs/INTELLIGENCE_GATEWAY.md`). Trip Brain's own call sites (`ai/trip_brain/discovery_adapters.py`) are unchanged — they still call each Discovery module's public `service.recommend()`/`check()`/`analyse()` method, unaware of what that module's provider is or how it obtained its data.
