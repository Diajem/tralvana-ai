...
Tralvana Travel does not "search and list" — it reasons about which option best fits the traveller, goal, budget, trip plan, and preferences, and explains why. Every Discovery module (Flight, Accommodation, Destination, Budget, Visa, and Weather & Safety) exists to turn a category of raw options into ranked, explainable recommendations. A consistent internal pipeline means a new engineer who has read one Discovery module already understands the shape of every other one.
...
| Weather & Safety Intelligence | `services/api/app/domains/weather/` | `ai/discovery/weather/` | ADR-016 |
