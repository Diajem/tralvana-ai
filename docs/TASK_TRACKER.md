...
| T-022 | Trip Brain Implementation | `planned` | high | Builds `docs/TRIP_BRAIN_ARCHITECTURE.md`/ADR-017 exactly as documented. Repoints `Intent.PLAN_TRIP` at Trip Brain instead of `TravelManager`. Explicitly does **not** touch `ai/manager/`, `ai/registry/`, or `ai/agents/{flight,hotel,budget,experience,visa}_agent.py` — they remain in place, uncalled, as a rollback path. See ADR-017's Decision and Alternatives Considered for why deletion is deferred to T-023 |
| T-023 | Legacy Orchestration Cleanup | `planned` | medium | Removes `ai/manager/`, `ai/registry/`, and the five placeholder agent files (`ai/agents/{flight,hotel,budget,experience,visa}_agent.py`) once T-022 has been fully implemented and verified. Blocked until T-022 is done — not a parallel or bundled effort |
